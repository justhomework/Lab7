...
function Coin(pos) { //constructor
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
Coin.prototype.type = "coin"; //declare type

function Portal(pos) { //constructor
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
...
Coin.prototype.act = function(step) { //make it move
...
Portal.prototype.act = function(step) { //make it move
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  Player.prototype.moveY = function(step, level, keys) {
    ...
    var newPos = this.pos.plus(motion);
    var resPos = new Vector (10,10)

    var obstacle = level.obstacleAt(newPos, this.size);
    var lava = level.obstacleAt(newPos, this.size);
...
Level.prototype.playerTouched = function(type, actor) { //what happens when player collides with it
  if (type == "coin") {
...
Portal.prototype.playerTouched = function(type,actor){
  if (type == "portal"){
    this.actors = this.actors.filter(function(other) {
      return other != actor;
  });
